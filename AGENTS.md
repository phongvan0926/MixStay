...
- **Tên tin & mô tả ra công khai phải qua `redactTitle()` / `redactPublicText()`** — chủ nhà gõ số nhà và SĐT vào đó, luật ẩn số nhà bị lách qua đường này.
...
- **Công cụ nghiệp vụ (tải ảnh / copy nội dung) KHÔNG được hiện cho khách vãng lai** — `ListingActionBar` mặc định `tools=false`, chỉ bật ở kho CTV. Bật nhầm ở trang công khai là để đối thủ tải sạch kho hàng.
