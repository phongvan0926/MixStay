...
- **Số điện thoại:** KHÔNG bao giờ dựng `tel:`/`zalo.me/` bằng cách nối chuỗi thô (`'tel:' + phone`). Dùng `telHref()`/`zaloHref()` của `lib/phone.ts` — chủ nhà hay ghi kèm tên ("Lâm 0394632595"), nối thẳng ra link chết. Thêm ô SĐT mới → validate bằng `checkPhone()`.
