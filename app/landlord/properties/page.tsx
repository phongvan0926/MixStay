...
export default function LandlordPropertiesPage() {
  ...
  // Group roomTypes by propertyId (lọc theo ô tìm kiếm nếu có)
  const rtByProperty = useMemo(() => {
    const q = search.trim().toLowerCase();
    const codeNorm = q ? normalizeListingCode(search).toLowerCase() : '';
    const map: Record<string, any[]> = {};
    roomTypes.forEach((rt: any) => {
      if (!rt.propertyId) return;
      if (q) {
        const code = (rt.listingCode || '').toLowerCase();
        const hit = rt.name?.toLowerCase().includes(q) || code.includes(q) || (codeNorm && code.includes(codeNorm));
        if (!hit) return;
      }
  ...
  return (
    <div>
      ...
      {/* Tìm theo mã tin / tên loại phòng */}
      <div className="mb-5">
        <input className="input-field w-full sm:max-w-sm"
          placeholder="Tìm theo mã tin (MS-…) hoặc tên loại phòng"
      ...
      <div className="space-y-6">
        {properties.map((p: any) => {
          ...
          if (search.trim() && rts.length === 0) return null; // ẩn tòa không có tin khớp tìm kiếm
