...
app/san-phong/[token]/ → TRANG THEO DÕI "SĂN PHÒNG" của KHÁCH (public theo token, không cần tài khoản, noindex): admin gửi link qua Zalo 1 lần, khách quay lại là thấy danh sách tin khớp LUÔN TƯƠI (force-dynamic). Dữ liệu từ getSavedSearchPageData (lib/seo-listings) — dùng chung bộ khớp lib/saved-search-match, KHÔNG bao giờ đưa SĐT/tên khách ra trang
...
app/api/rooms/public → hỗ trợ ?sort=price_asc|price_desc|newest|area_desc|views_desc|**deal** + ?uni=<short HANOI_UNIVERSITIES> — tính khoảng cách server-side (KHÔNG trả lat/lng), sort gần nhất, trả distanceKm + viewCount + dealPercent
                       `?mixUnseen=1` (trang 1, không sort): thay 2 SLOT CUỐI bằng tin CHƯA AI XEM (viewCount=0, có ảnh, luân phiên mỗi giờ) — 114/510 tin hiệu lực đang 0 lượt xem vì mọi danh sách xếp mới-nhất-trước (đo 19/08/2026). Chỉ trang chủ (FeaturedRooms tab Mới đăng) truyền tham số này
...
app/api/saved-searches → "Săn phòng": khách (không cần tài khoản) để lại tiêu chí + SĐT; admin quản lý ở /admin/leads. Mỗi yêu cầu có `token` (sinh khi tạo, sinh LƯỜI qua ?matchesFor cho bản ghi cũ) → link theo dõi /san-phong/<token>; ?matchesFor trả kèm token. KHỚP TIN 3 ĐƯỜNG qua lib/saved-search-match.ts: khách vừa đăng ký → quét NGƯỢC kho có sẵn ngay; tin mới duyệt → quét xuôi (PUT /api/rooms); cron quét lại hằng ngày (chỉ báo phần tin MỚI sau lần báo gần nhất). ?matchesFor=<id> trả danh sách tin khớp cho 1 khách
...
app/api/rooms       → thêm `?unseen=true` (viewCount=0) — chip "💎 Chưa ai xem" ở /broker/inventory: hàng chưa CTV nào cạnh tranh, mang đi đăng lại dễ nổi nhất
...
components/admin/IssueBanner.tsx → Dải vàng "đang lọc theo việc cần xử lý nào" + nút Bỏ lọc, hiện ở /admin/rooms và /admin/properties khi vào từ thẻ trên Tổng quan. Gồm cả `property-pending` = tin đã duyệt mà TÒA chưa duyệt (khách không thấy)
components/leads/CloseDealModal.tsx → Hỏi "Khách này có thuê không?" ngay khi đổi lead sang 🟢 Đã dẫn xem → ghi thẳng Deal. Client CHỈ gửi giá + hoa hồng + ghi chú; phòng/khách/CTV do SERVER đọc từ lead. "Chưa chốt được" là nút to ngang nút kia — ép nhập tiền thì admin né luôn việc đổi trạng thái
...
lib/coords.ts       → Client-safe: BÓC TOẠ ĐỘ từ thứ admin DÁN VÀO (link Google Maps @lat,lng / !3d!4d / ?q= / cặp số trần, tự nhận ra cả khi dán ngược lat↔lng), kiểm khung Hà Nội, coordsError() nói rõ sai chỗ nào bằng tiếng Việt. Dùng ở PropertyForm (ô 📍 Vị trí trên bản đồ) + API properties
...
- deals: id, roomTypeId, **brokerId?** (NULL = công ty tự chốt, khách không qua CTV nào), **viewingRequestId?** (@unique — lead sinh ra deal này; một lead chỉ ghi được MỘT deal), dealPrice, commissionTotal, commissionBroker, commissionCompany, status (PENDING/CONFIRMED/PAID/CANCELLED)
- share_links: id, roomTypeId?, brokerId, token (unique), viewCount, isSystem, isActive, expiresAt
- saved_searches: … + `token` (@unique, nullable — link theo dõi công khai /san-phong/<token>; sinh khi tạo mới, sinh lười cho bản ghi cũ)
...
- **🚫 BẪY DUYỆT TÒA — duyệt tin KHÔNG đủ để tin hiện ra khách.** `PUBLIC_ROOM_WHERE` bắt buộc `property.status='APPROVED'`, nên tin `isApproved=true` thuộc tòa `PENDING` là **vô hình** ở mọi mặt tiền (trang chủ, `/phong`, bản đồ, sitemap, `/tin/[id]`). Đo 25/08/2026: 73 tin nằm im sau 43 tòa chờ duyệt, cũ nhất 8 ngày — vì bảng duyệt tin chỉ hiện tích xanh của riêng cái tin. **Bất kỳ màn hình nào cho phép duyệt/sửa tin đều phải hiện trạng thái TÒA kèm theo**, và mọi thẻ đếm "chờ duyệt" phải nói ra **hậu quả** (số tin bị giam), không chỉ số bản ghi. Điều kiện dùng chung: `{ isApproved:true, status:{in:['AVAILABLE','UPCOMING']}, property:{status:'PENDING'} }` — dựng ở `/api/admin/overview` (`trappedRooms`) và `GET /api/rooms?issue=property-pending`, hai chỗ phải khớp nhau.
...
- **Ghi deal phải hỏi ĐÚNG LÚC người ta biết câu trả lời.** Sau 2 tháng vận hành kho deal trống trơn (0 bản ghi / 44 lead) — không phải vì không chốt được khách, mà vì chốt xong phải mò sang trang Giao dịch nhập tay lại từ đầu. Nay `POST /api/deals` nhận `viewingRequestId` và tự đọc phòng/khách/CTV **từ lead ở server** (client chỉ được quyết tiền); modal bật ngay khi đổi lead sang 🟢 Đã dẫn xem. Thêm luồng ghi doanh thu mới thì giữ đúng nguyên tắc này: hỏi tại chỗ, không bắt nhập lại thứ hệ thống đã biết, và luôn có đường "chưa chốt".
...
- **Thông báo phải gộp theo NGƯỜI NHẬN, không theo từng bản ghi.** Admin thao tác theo ĐỢT (duyệt cả tòa 11 tin liền tay), nên "1 thông báo cho mỗi cặp (tin × khách)" biến một đợt duyệt thành hàng trăm tin nhắn gần y hệt — đo 25/08/2026: 103 thông báo/ngày, hộp thư dồn 755 mục chưa đọc và cái chuông **mất hẳn tác dụng cảnh báo** (lead nóng nằm lẫn trong đống rác). Mọi đường sinh notification hàng loạt phải: gộp thành MỘT tin mỗi người nhận (kiểu *"kho có N tin khớp: …"*), và tôn trọng **`MATCH_NOTIFY_COOLDOWN_MS`** (6h) tính theo TỪNG người nhận. Không mất dữ liệu vì lượt báo sau dùng `since: lastMatchedAt` để đếm bù.
- **Đừng chép lại bộ luật nghiệp vụ đã có — gọi vào file dùng chung.** `PUT /api/rooms` từng chép riêng luật khớp "săn phòng" thay vì gọi `lib/saved-search-match.ts`, và bản chép **quên `hasCriteria()`** → 3 khách bỏ trống tiêu chí khớp MỌI tin suốt nhiều tuần. Bản chép luôn tụt lại sau bản gốc; sửa bản gốc không lan sang bản chép. Đường khớp "săn phòng" chỉ được đi qua `roomMatchesSearch()` / `roomWhereForSearch()` / `matchAndNotify()`.
...
- **Geocode tự động KHÔNG tìm được khu đô thị mới ở Việt Nam — phải có đường ghim TAY.** Đo 18/08/2026: "Khu đô thị mới HUD Vân Canh" (17 tin) và "66 Nam 32" (1 tin) trượt sạch ở CẢ Nominatim lẫn Overpass — OSM không hề có đối tượng nào tên như vậy, nên `scripts/geocode-properties.js` chạy lại bao nhiêu lần cũng vẫn trượt. Ô ghim tay ở `PropertyForm` (nhận link Google Maps, xem `lib/coords.ts`) là lối duy nhất. ĐỪNG "sửa" bằng cách nới geocode cho nó rơi về tâm quận — pin sai còn tệ hơn không có pin.
- **Client GỬI toạ độ thì server KHÔNG được geocode đè.** Trước 18/08/2026, `PUT /api/properties` cứ thấy địa chỉ đổi là re-geocode: admin ghim tay xong, lần sau sửa mỗi lỗi chính tả trong địa chỉ là pin bị Nominatim đoán lại rồi ghi đè mất. Nay chỉ geocode khi client KHÔNG gửi trường toạ độ; gửi `null` tường minh = BỎ ghim.
- **Toạ độ do người dùng gửi phải kiểm khung Hà Nội ở SERVER** (`inHanoi()` trong `lib/coords.ts`), không chỉ chặn ở form — dán nhầm toạ độ Paris mà lọt vào DB thì tòa nhà biến mất khỏi bản đồ theo kiểu không ai hiểu nổi.
