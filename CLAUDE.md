...
scripts/backup-storage.js → BACKUP kho ảnh/video Supabase Storage về Ổ SSD SAMSUNG 120GB gắn trong máy — `/srv/data/MixStay/storage` (backup của Supabase KHÔNG gồm file Storage!). DỪNG hẳn nếu ổ chưa mount (tránh ghi nhầm làm đầy ổ hệ thống). Tăng dần (bỏ qua file đã đủ dung lượng), chỉ đọc, ghi manifest.json. `--check` đối chiếu không tải, `--dest <thư mục>` đổi đích. scripts/install-backup-cron.sh cài lịch 3h sáng CN hằng tuần (`--remove` để gỡ). LƯU Ý: `/srv/data/backup` là repo restic của hệ thống — KHÔNG đụng vào
