...
scripts/backup-storage.js → BACKUP kho ảnh/video Supabase Storage về máy (backup của Supabase KHÔNG gồm file Storage!). Tăng dần (bỏ qua file đã đủ dung lượng), chỉ đọc, ghi manifest.json. `--check` đối chiếu không tải, `--dest <thư mục>` đổi đích. scripts/install-backup-cron.sh cài lịch chạy 3h sáng CN hằng tuần (`--remove` để gỡ)
