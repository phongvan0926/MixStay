...
### v9.71 — 2026-09-03 (chuông thông báo tự đầu độc — 755 mục chưa đọc, gộp lại còn 1/6)
- **🔴 Triệu chứng:** hộp thông báo của admin dồn **755 mục chưa đọc**, trong đó **673/849 là "săn phòng"** — cái chuông mất hẳn tác dụng cảnh báo, lead nóng nằm lẫn trong đống tin trùng.
- **Gốc rễ — hai đường xử lý cùng một việc theo hai cách khác nhau.** `lib/saved-search-match.ts` (cron hằng ngày) làm ĐÚNG: gộp một thông báo mỗi khách, *"minh châu 0344801880 — kho có 10 tin khớp: MS-… · MS-…"*. Nhưng `PUT /api/rooms` **chép riêng một bản luật** rồi tự dựng notification theo **từng cặp (tin × khách)**. Admin duyệt liền tay cả tòa 11 tin là 11 lượt báo gần y hệt nhau — đo 25/08/2026: **103 thông báo trong một ngày**.
- **Lỗi thứ hai trong cùng khối đó:** bản chép tay **quên gọi `hasCriteria()`**, nên **3/19 khách đang săn bỏ trống hết tiêu chí** (Tấn Trực, Bùi Linh, sang) **khớp MỌI tin** — mỗi lượt duyệt cộng thêm 3 thông báo rác. `hasCriteria` đã tồn tại sẵn đúng để chặn ca này, chỉ là đường này không đi qua nó.
- **Sửa:** xoá bản luật chép tay, `PUT /api/rooms` nay gọi thẳng `roomMatchesSearch()` + `matchAndNotify()` của bộ khớp dùng chung → tự có `hasCriteria`, tự gộp *"kho có N tin khớp"*. Thêm **`MATCH_NOTIFY_COOLDOWN_MS` = 6 giờ**: khách vừa được báo thì đợt duyệt này không báo lại — **không mất tin nào**, vì lượt báo kế tiếp dùng `since: lastMatchedAt` nên vẫn đếm đủ phần tin mới.
- **Đo trên dữ liệu thật, duyệt 3 tin:** cách cũ **12 thông báo** → cách mới **2** (giảm **83%**). Kiểm riêng khoảng lặng: duyệt tin thứ 3 khớp Huy Hùng ngay sau khi anh ấy vừa được báo → **không sinh thông báo cho anh ấy** (`lastMatchedAt` giữ nguyên), nhưng khách khác (minh châu) **vẫn được báo** — khoảng lặng tính theo TỪNG khách, không phải chặn cả lượt.
- **Chọn ca thử không đụng khách:** cả 3 tin đều thuộc tòa **đang chờ duyệt**, nên duyệt tin cũng không làm gì hiện thêm với khách. **Dữ liệu thử đã hoàn nguyên đúng 3/3**: 849 → 849 thông báo, 17 → 17 tin chờ duyệt, 0 bản ghi `lastMatchedAt` lệch.
- Màn `/admin/leads` tab Săn phòng vốn đã hiện đúng *"Chưa rõ tiêu chí — gọi hỏi"* cho 3 khách nói trên, nên không phải sửa gì thêm ở giao diện.
- **Chưa làm (việc của chủ dự án):** 755 thông báo cũ vẫn đang nằm đó — dọn hộp thư là xoá/đánh dấu dữ liệu production, chờ chủ dự án quyết.

### v9.70 — 2026-08-25 (gỡ BẪY DUYỆT TÒA — 73 tin đã duyệt đang vô hình với khách)
- **🔴 Lỗi thiết kế luồng, không phải lỗi người vận hành.** Admin vẫn duyệt tin đều đặn (754/754 tin đã duyệt, nhật ký thao tác hôm nay đầy `approve roomType`). Nhưng `PUBLIC_ROOM_WHERE` (`lib/seo-listings.ts`) bắt buộc `property.status = 'APPROVED'` — **tòa chưa duyệt thì tin duyệt rồi vẫn không ai thấy**: không trang chủ, không `/phong`, không bản đồ, không sitemap. Đo 25/08/2026: **73 tin đã duyệt + còn phòng nằm im sau 43 tòa chờ duyệt**, tin cũ nhất chờ 8 ngày, tòa nặng nhất giam 11 tin.
- **Vì sao không ai phát hiện ra:** bảng `/admin/rooms` chỉ hiện dấu **"✓ Đã duyệt"** của riêng cái tin, **không hề nhắc tới trạng thái tòa**. Admin bấm duyệt, thấy tích xanh, tin là xong việc. Còn thẻ trên Tổng quan thì ghi *"Tòa chờ duyệt: 48"* — nghe như việc hành chính, không nói ra cái giá phải trả.
- **Sửa ở đúng chỗ người ta đang nhìn:** mỗi hàng ở `/admin/rooms` có tòa chưa duyệt giờ hiện dải đỏ **"🚫 Tòa chưa duyệt — tin đã duyệt nhưng khách VẪN CHƯA THẤY"** kèm nút **"✓ Duyệt tòa ngay"** bấm tại chỗ (gọi `PUT /api/properties {status:'APPROVED'}`, cùng đường với nút Duyệt ở trang Tòa nhà, có gate `APPROVE_LISTINGS`).
- **Thẻ Tổng quan nói hậu quả thay vì nói con số:** "Tòa chờ duyệt 48" thêm dòng phụ *"🚫 Đang giam 73 tin — khách không thấy"* và **đổi sang tông đỏ** khi đang giam tin; thêm thẻ **"🚫 Tin bị tòa giam"** dẫn thẳng tới `/admin/rooms?issue=property-pending`. Trang Tòa nhà: mỗi tòa PENDING hiện *"🚫 Đang giam N tin đã duyệt"* để duyệt tòa nhiều tin trước.
- **Đếm và lọc dựng bằng một điều kiện:** `trappedRooms` trong `/api/admin/overview` và nhánh `issue=property-pending` trong `GET /api/rooms` dùng chung định nghĩa `{ isApproved:true, status:{in:['AVAILABLE','UPCOMING']}, property:{status:'PENDING'} }` — đo trên dữ liệu thật **73 = 73**, thẻ và danh sách không lệch nhau.
- **Kiểm bằng phiên admin thật:** thẻ hiện → bấm sang danh sách đã lọc đúng 73 tin → dải `IssueBanner` giải thích → 20/20 hàng trang 1 có cảnh báo + nút. Nút "Duyệt tòa ngay" kiểm bằng cách **chặn request lại** (`page.route`) để đọc payload mà **không ghi gì vào production**: gửi đúng `{"id":"…","status":"APPROVED"}`, toast báo đúng. **48 tòa đang chờ vẫn giữ nguyên trạng thái** — duyệt tòa là quyết định của chủ dự án, code chỉ làm cho nó hiện rõ và bấm được một chạm.
- Rà thêm: 0 tin kẹt vì tòa `REJECTED` hoặc `isActive=false` — toàn bộ 73 tin đều kẹt vì `PENDING`.

### v9.69 — 2026-08-19 (ghi giao dịch NGAY TRÊN LEAD — mở đường cho dòng doanh thu đầu tiên)
- **🔴 Vấn đề:** 44 lead đã vào, **0 giao dịch được ghi** sau 2 tháng vận hành. Không phải vì không chốt được khách — mà vì **chốt xong không có chỗ ghi**: muốn ghi phải mò sang trang Giao dịch rồi nhập tay lại từ đầu (phòng nào, khách tên gì, SĐT bao nhiêu). Không ai làm thế. Chừng nào chưa có dòng doanh thu thì mọi con số khác (tỉ lệ chốt, hoa hồng CTV, hiệu quả từng kênh) đều không tính được.
- **Hỏi đúng lúc người ta biết câu trả lời:** đổi lead sang **🟢 Đã dẫn xem** là thời điểm DUY NHẤT biết kết quả → modal bật ngay: *"Khách này có thuê không?"* với giá thuê điền sẵn theo giá rao, hoa hồng mặc định nửa tháng tiền thuê, ô ghi chú. Xong là có Deal, không phải mở trang nào khác.
- **Không ép:** *"Chưa chốt được"* là nút **to ngang** nút kia, cộng thêm "Để sau". Bắt nhập tiền mới cho đổi trạng thái thì admin né luôn việc đổi trạng thái — mất cả dữ liệu lẫn quy trình.
- **🔒 Client chỉ được quyết TIỀN.** `POST /api/deals` nhận `viewingRequestId` rồi tự đọc **phòng nào / khách nào / CTV nào từ lead ở SERVER**; body có nhét `roomTypeId`/`customerName` giả cũng bị bỏ qua. Đã thử: gửi `customerName:"HACKER"` + `roomTypeId:"gia-mao"` → server không dùng.
- **Schema:** `Deal.brokerId` chuyển **nullable** — giao dịch **công ty tự chốt** (khách vào thẳng web, không qua CTV) là chuyện có thật và đang là 44/44 lead; bắt buộc phải có CTV thì không ghi nổi deal nào. Thêm `Deal.viewingRequestId` **@unique** → một lead chỉ ghi được MỘT deal (bấm nhầm 2 lần không đẻ ra doanh thu ảo), và bảng lead hiện lại badge **💰 Đã chốt N triệu**, lead đã chốt thì **không hỏi lại**.
- Trang Giao dịch: deal không có CTV hiện **🏢 Công ty tự chốt** thay vì ô trống trông như thiếu dữ liệu.
- **Kiểm bằng phiên admin + phiên CTV thật:** ghi một deal thật qua giao diện → DB đúng (phòng/khách lấy từ lead, hoa hồng chia 60/40 theo Cài đặt, trạng thái CONFIRMED vì admin ghi, lead chuyển DONE) → badge hiện → chọn lại DONE **không** hỏi lần hai → POST lại cùng lead trả **409**. Guard: CTV ghi cho lead người khác **403**, chưa đăng nhập **401**, lead không tồn tại **404**, thiếu cả hai id **400**. **Dữ liệu thử đã dọn sạch** — xoá đúng deal có ghi chú đánh dấu (không dùng `deleteMany` trần) và trả 2 lead về đúng trạng thái cũ; phân bố lead sau dọn khớp chính xác lúc đầu (NEW=29 · CONTACTED=9 · CANCELLED=6).
...
- **🔴 Lỗ hổng thật sự: `PropertyForm` KHÔNG có ô toạ độ nào** (0 dòng nhắc `latitude`). Thẻ việc trên Tổng quan bấm vào là tới đúng danh sách 2 tòa… rồi bó tay — vi phạm chính quy tắc "mọi việc hiện ra phải tới được chỗ xử lý".
- **Thêm ô 📍 Vị trí trên bản đồ vào form tòa nhà.** Nhận thứ admin CÓ SẴN trong tay: dán thẳng link Google Maps (`@lat,lng`, `!3d…!4d…`, `?q=…`) hoặc gõ hai số. `lib/coords.ts` bóc toạ độ, **tự nhận ra cả khi dán ngược lat↔lng** (khoảng lat 20,5–21,5 và lng 105,2–106,2 của Hà Nội không giao nhau nên đảo là nhận ra chắc chắn). Kèm nhãn trạng thái (✅ đã ghim / ⚠️ chưa ghim, tòa KHÔNG hiện trên bản đồ), link **Xem thử** mở đúng điểm vừa ghim, nút **Bỏ ghim**, và nút **🎯 Tìm theo địa chỉ** gọi geocode — không ra thì nói thẳng "khu đô thị mới thường không có trên bản đồ, phải ghim tay" chứ không để admin bấm lại chờ phép màu.
- **Báo lỗi nói rõ sai ở đâu**, không im lặng nuốt: dán link rút gọn `maps.app.goo.gl` → chỉ cách lấy link đầy đủ; dán toạ độ Paris → "nằm ngoài Hà Nội"; dán chữ → ví dụ đúng định dạng.
- **🐛 Vá lỗi ghi đè pin (có sẵn từ trước):** `PUT /api/properties` cứ thấy địa chỉ đổi là re-geocode và **ghi đè toạ độ đã ghim tay** — admin ghim xong, lần sau sửa mỗi lỗi chính tả trong địa chỉ là mất pin. Nay client gửi toạ độ thì server tôn trọng tuyệt đối; gửi `null` tường minh = bỏ ghim; và toạ độ gửi lên **được kiểm khung Hà Nội ở SERVER** (400 nếu ngoài khung), không chỉ chặn ở form.
- **Đã ghim xong cả 2 tòa, kho còn 0 tòa thiếu toạ độ** (thẻ Tổng quan `propsNoGeo` 2 → 0), 18 tin lên bản đồ:
  - `HUD Vân Canh` → **21.039103, 105.730912** — trọng tâm 4 đối tượng OSM mang đúng tên khu ("Đối diện số 20 LK 39 Khu đô thị Vân Canh" trên đường Kim Thìa, Công viên A/B/C Khu chung cư Vân Canh). Mức chính xác: **cỡ khu đô thị (~±700m)**, đúng tầm của một địa chỉ không có số nhà.
  - `66 Nam 32` → **21.065000, 105.710500** — tra ra đây là **KĐT Westpoint Nam 32** (chủ đầu tư Lũng Lô 5), bám Quốc lộ 32 thuộc Trạm Trôi/Đức Giang/Đức Thượng, tức bờ **NAM** QL32 đối diện Lideco ("Bắc 32"). Đối chiếu hình học OSM: QL32 tại kinh độ Lideco nằm ở vĩ độ 21.06714 → ghim ~230m về phía nam. Mức chính xác **cỡ thị trấn (~±500m)** — nên nhờ chủ nhà xác nhận lại, sửa mất 30 giây bằng ô mới.
- **Phát hiện kèm theo (chưa sửa, để chủ dự án quyết):** toạ độ đang lưu của tòa `Khu đô thị Lideco-Trạm Trôi` là **21.06818, 105.70990** — đúng bằng **tâm thị trấn Trạm Trôi** do Nominatim trả về, không phải khu Lideco (tâm đa giác OSM ở 21.070405, 105.712786, lệch ~400m). Và tòa `15LK26 Khu Đô Thị Vân Canh` đang ghim ở **phường Xuân Phương, Nam Từ Liêm** — cách khu Vân Canh thật ~1,5km, lại còn lưu sai quận.
- **Kiểm bằng phiên admin thật (Playwright + JWT tự ký tại máy):** vào thẻ việc → mở tòa → dán rác/toạ độ ngoài Hà Nội/link Google Maps đầy đủ → ghim → Lưu (PUT 200) → đối chiếu DB. Kiểm cả 3 hành vi dễ hỏng: mở lại form pin hiện đúng, **bấm Lưu mà không đụng gì thì pin còn nguyên**, bỏ ghim ra `null`. Bộ phân tích toạ độ có 13 ca kiểm thử, đạt hết.

### v9.66 — 2026-08-18 (thanh nav: logo nguyên bản + nút bản đồ đủ chữ trên điện thoại)
- **Logo trở lại nguyên bản ở MỌI bề rộng** — không cắt, không xếp dọc, không bóp ngang. Bản xếp dọc (biểu tượng trên, chữ "MixStay" dưới, cắt bằng `object-cover`) sinh ra hồi nút bản đồ còn kèm icon 🗺️ và cần nhường chỗ; bỏ icon xong nút hẹp đi nên trả logo về đúng khổ gốc. Đo lại tỷ lệ **3.01 đúng bằng tỷ lệ gốc của `logo-nav.png`** ở cả 10 bề rộng.
- **Nút bản đồ hiện ĐỦ CHỮ "Tìm theo bản đồ" trên điện thoại**, thay vì rút gọn thành "Bản đồ" — chữ cụt không nói được đây là tìm phòng trên bản đồ.
- **🔴 Hai thứ đó cộng lại chiếm 226px nên phải bỏ bớt mục khác.** Đo ở 390px: logo 84 + nút 138 + "Đăng nhập" 93 + "Đăng ký" 77 = **402px trong khi chỉ có 358px dùng được**; máy 320px thiếu tới 114px. **Chủ dự án chọn giữ "Đăng nhập", bỏ "Đăng ký miễn phí"** khỏi nav điện thoại (chân trang đã có sẵn 4 link đăng ký theo từng vai trò).
- **🐛 Sửa nốt lỗi ĐÈ NÚT khi đã đăng nhập.** Ở 390px, nhóm phải của phiên đăng nhập (Quản lý + ảnh đại diện + Đăng xuất) rộng 166px làm nút "Quản lý" **che mất 34px nút bản đồ — chữ chỉ còn "Tìm theo bả…"**. Nay điện thoại chỉ giữ nút "Quản lý"; ảnh đại diện + tên + Đăng xuất hiện từ 768px, riêng TÊN từ 1024px (đăng xuất vẫn có ở topbar trang quản lý).
- **⚠️ Lỗi CÓ SẴN được phát hiện nhờ lượt đo này:** khoảng máy tính bảng **640–768px khi đã đăng nhập vốn đã đè nhau từ trước** (nhóm phải đầy đủ 451px + nhóm trái 282px = 733px, trong khi 768px chỉ dùng được 720px). Không phải do thay đổi hôm nay — bản cũ ở 640px còn thừa nhiều hơn — nhưng đã sửa luôn.
- **⚠️ Bài học đo đạc:** `justify-between` **không** bảo vệ khỏi tràn, nó **giấu** tràn. Hai nhóm quá khổ đè lên nhau mà `scrollWidth === clientWidth` (không có thanh cuộn ngang) và bounding box của NHÓM vẫn báo "hở 0px", vì con bên trong có `shrink-0` nên tràn ra ngoài hộp cha. Phải đo bằng toạ độ phần tử CON hai bên. Đây là lý do lượt kiểm đầu tiên suýt kết luận "đạt".
- **Đã đo 4 vai trò × 10 bề rộng** (khách chưa đăng nhập / chủ nhà / admin / CTV × 320→1280px) bằng Playwright có mock `/api/auth/session`, cộng 6 trang công khai dùng chung nav: không chỗ nào đè, không tràn ngang, logo giữ đúng tỷ lệ, vùng bấm ≥44px trên điện thoại.
